...
> 本项目仅用于信息整理和投资研究，不构成投资建议，不执行自动交易。盘中估值并非基金管理人公布的正式净值，历史模型概率也不保证未来结果。
...
- 走势概率：使用 XGBoost 估算下一交易日上涨概率，展示历史区间、主要因子与滚动回测结果。
...
- 后端：FastAPI、SQLite、Pydantic、XGBoost
...
- `007818` 的下一交易日概率优先使用目标 ETF `515880` 的前复权日线；行情节点不可用时回退到已同步的基金官方净值历史。
- 预测特征只使用预测日及之前的数据，并通过扩展窗口滚动回测对比简单多数基准；没有超过基准时强制显示“方向不明”。
- DeepSeek 不参与数值预测，只用于 AI 日报文字分析；预测概率由本地 XGBoost 模型计算并缓存在 SQLite。
...
| `GET /api/predictions/funds/{code}` | 下一交易日上涨概率与滚动回测 |
| `GET /api/predictions/funds/{code}?refresh=true` | 强制刷新历史行情并重新训练 |
...
这是正常的风险保护。当滚动回测准确率没有超过简单基准，或 AUC 低于最低有效标准时，页面不会把原始模型概率包装成方向信号。可查看卡片中的准确率、基准和 AUC，并等待更多历史数据后重新计算。
